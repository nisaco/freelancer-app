...
const ArtisanDashboard = () => {
  ...
  if (loading) return (
    <div className="h-screen flex items-center justify-center bg-white font-black uppercase text-blue-600 animate-pulse tracking-[0.5em]">
      Loading Artisan Hub...
    </div>
  );
      ...
      <div className="min-h-screen bg-[#F8FAFC] pb-20">
        ...
        <div className="max-w-6xl mx-auto px-6 pt-12">
          ...
          <div className="flex justify-between items-end mb-10">
            <div>
              <h1 className="text-5xl font-black text-gray-900 tracking-tighter uppercase">Artisan Dashboard</h1>
              <p className="text-[10px] font-black text-blue-600 uppercase tracking-[0.4em] mt-2">Manage your business & wallet</p>
            </div>
            ...
            <div className="h-[200px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={chartData}>
                  ...
                  <Area type="monotone" dataKey="amount" stroke="#2563eb" fill="#dbeafe" strokeWidth={5} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </div>
              ...
              <div key={job._id} className="bg-white p-8 rounded-[2.5rem] border border-gray-50 flex flex-col justify-between shadow-sm">
                <div className="flex justify-between items-start mb-6">
                  <div>
                    ...
                    <p className="text-[9px] font-bold text-gray-400 uppercase tracking-[0.2em] mt-1">Client: {job.client?.username || 'Guest'}</p>
                  </div>
                  <span className={`px-4 py-1.5 rounded-full text-[8px] font-black uppercase tracking-widest ${job.status === 'completed' ? 'bg-blue-50 text-blue-600' : 'bg-green-50 text-green-600'}`}>
                    {job.status}
                  </span>
                </div>
                <div className="flex items-center justify-between pt-6 border-t border-gray-50">
                  ...
                  {job.status === 'paid' && (
                    <motion.button 
                       ...
                       className="px-6 py-3 bg-gray-900 text-white text-[9px] font-black uppercase tracking-widest rounded-xl hover:bg-blue-600 transition-all shadow-lg shadow-gray-200"
                    >
                      Mark Finished
                    </motion.button>
                  )}
...
const SettingsDrawer = ({ user, setUser, onClose, API_BASE }) => {
  ...
  return (
    <>
      ...
      <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', damping: 25 }} className="fixed top-0 right-0 h-full w-full max-w-md bg-white z-[200] p-10 flex flex-col shadow-2xl">
